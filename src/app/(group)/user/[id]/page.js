import loadSingleUserData from '@/utils/loadSingleUserData';
import React from 'react';

const userId = async ({ params }) => {

    const { id, name, email } = await loadSingleUserData(params.id);

    return (
        <div>
            <article className="rounded-xl bg-white p-4 ring ring-indigo-50 sm:p-6 lg:p-8 mx-20 my-4">
                <div className="flex items-start sm:gap-8">
                    ...
                    <div>
                        <strong
                            className="rounded border border-indigo-500 bg-indigo-500 px-3 py-1.5 text-lg font-medium text-white"
                        >
                            {id}.  {name}
                        </strong>
